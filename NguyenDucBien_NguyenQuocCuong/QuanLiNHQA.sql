...
IF EXISTS(SELECT NAME FROM SYS.DATABASES WHERE NAME = 'QuanLiNNHQA')

DROP DATABASE QuanLiNNHQA

ELSE

CREATE DATABASE QuanLiNNHQA

USE QuanLiNNHQA
GO

CREATE TABLE BillCopy
(
	BillID INT NOT NULL,
...
go
---------------
CREATE PROC StoreProc_InsertBillCopy @billid int , @tablename nvarchar(100), @checkin datetime, @checkout datetime, @discount int , @totalprice int
AS BEGIN
